...
NdtScanMatcher::AlignResult NdtScanMatcher::align(
  ...
  const builtin_interfaces::msg::Time & now)
{
  ...
  result.loaded_pcd_map =
    install_map_update(initial_pose_in_map_frame.pose.pose.position, result.diagnostics);
  ...
  ndt_ptr_.with([&](auto & ndt_ptr) {
    search = search_initial_pose(
      param_.pose_initialization, *ndt_ptr, scan, initial_pose_in_map_frame, now);
  });
